...
const topics = [
  "شلونكم اليوم؟",
  "شنو الأخبار؟",
  "أحد جرب المكان الجديد؟",
  "منو بعده صاحي؟ 😂",
  "شنو رأيكم بالحياة بفنلندا؟",
  "مين يعرف مكان هادي للقهوة؟ ☕",
  "شلون كان يومكم؟",
  "أحد عنده اقتراح لفيلم حلو؟",
  "شنو آخر شي ضحككم؟ 😂",
  "شنو خطتكم للويكند؟",
  "أحتاج رأيكم بشغلة صغيرة.",
  "والله الجو اليوم يجنن 😄",
  "شكراً على المعلومة، فادتني هواي.",
  "حلوة الفكرة، خلونا نبدأ.",
  "ههههه نفس السؤال جان ببالي 😂",
  "نورتوا الغرفة ❤️",
  "إذا تعرفون خبر جديد شاركونا.",
];

function profile(id: number, displayName: string): Profile {
...
export function getDailyDemoMessages(_roomId: string, _count = 0): MessageWithAuthor[] {
  return [];
}
export const DEMO_MESSAGES = (_roomId: string): MessageWithAuthor[] => [];
export function getDemoMembers(limit = 120): Profile[] {
  return getDailyDemoProfiles(limit);
}
export function getDemoTopic(seed: number): string {
