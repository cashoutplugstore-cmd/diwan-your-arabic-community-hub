...
function rememberAiText(roomId: string, text: string) { const list = recentAiTexts.get(roomId) ?? []; const next = [...list, text].slice(-12); recentAiTexts.set(roomId, next); return next; }
...
function uniqueAiText(roomId: string, memberIndex: number, context: string[], fallbackPool: string[]) {
  const member = aiMembers[memberIndex % aiMembers.length]!;
  const attempts = 8;
  for (let attempt = 0; attempt < attempts; attempt++) {
    const generated = buildAiConversation(member, [...context, ...fallbackPool.slice(attempt % fallbackPool.length, (attempt % fallbackPool.length) + 1)]).text || fallbackPool[(attempt + memberIndex) % fallbackPool.length]!;
    if (!isTooSimilar(roomId, generated)) { rememberAiText(roomId, generated); return generated; }
  }
  const fallback = fallbackPool.find((text) => !isTooSimilar(roomId, text)) ?? `${member.name}: ${fallbackPool[(memberIndex + Math.floor(Math.random() * fallbackPool.length)) % fallbackPool.length]!}`;
...
export function buildDemoEntryMessages(roomId: string): MessageWithAuthor[] {
  ...
  return [0, 1, 2].map((index) => {
    ...
    const text = index === 0 ? greetings[(now + index) % greetings.length]! : uniqueAiText(roomId, aiMembers.indexOf(member), [seed], pool);
...
export function buildDemoReply(roomId: string, realMessage: string): MessageWithAuthor | null {
  ...
  const pool = roomPools[roomKey(roomId)] ?? roomPools["general"]!;
  const generated = isGreeting ? uniqueAiText(roomId, aiMembers.indexOf(member), [realMessage], greetings) : uniqueAiText(roomId, aiMembers.indexOf(member), [realMessage], [...pool, ...followUps]);
