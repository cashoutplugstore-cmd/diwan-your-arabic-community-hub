...
const roomPools: Record<string, string[]> = {
  helsinki: [
    "شلون المواصلات اليوم؟ 🚋",
    "أحد يعرف كوفي هادي بهلسنكي؟ ☕",
    "الجو اليوم غريب 😂",
    "مين رايح للسنتر اليوم؟",
    "أحد جرب مكان جديد للأكل؟ 🍜",
  ],
  finland: [
    "شنو أخباركم بفنلندا؟ 🇫🇮",
    "الشتاء بعده بعيد لو لا؟ 😂",
    "أحد يعرف مكان حلو للويكند؟",
    "شنو أفضل مدينة للمعيشة برأيكم؟",
    "أحد عنده تجربة ويا الدراسة هنا؟ 📚",
  ],
  iraq: [
    "شلونكم أهل العراق ❤️🇮🇶",
    "اشتقت للأكل العراقي 😂",
    "منو بعده يحب الشاي بالهيل؟ ☕",
    "شنو أحلى أكلة عراقية برأيكم؟",
  ],
  students: [
    "منو عنده امتحان هالأسبوع؟ 📚",
    "أحد يعرف مصادر زينة للدراسة؟",
    "وين أحصل مكتبة هادئة؟",
    "خلصتوا واجباتكم؟ 😅",
  ],
  friends: [
    "شنو مسوين اليوم؟ 😄",
  ...
  general: [
    ...
    "أحد صاحي بهالوقت؟ 😂",
    "وين أحصل قهوة زينة؟ ☕",
    "والله اليوم الجو حلو 😄",
    "منو عنده اقتراح لفيلم؟ 🎬",
    "شنو خطتكم للويكند؟",
    "أحد جرب مطعم جديد؟",
    "اشتقت للسوالف هنا 😄",
  ],
...
const followUps = [
  "إي والله 😄 شنو رأيك؟",
  "ههههه مضبوط 😂",
  "اتفق وياك، كمل السالفة 🌷",
  "زين! منو عنده تجربة ثانية؟",
  "حلوة السالفة 😄",
...
function aiAuthor(roomId: string, salt = 0) {
  ...
  return {
    ...
    author: {
      ...fallback,
      id: fallback.id ?? `demo-${roomId}-${cursor + salt}`,
...
function rememberAiText(roomId: string, text: string) {
  const list = recentAiTexts.get(roomId) ?? [];
  const next = [...list, text].slice(-12);
  recentAiTexts.set(roomId, next);
  return next;
}
function isTooSimilar(roomId: string, text: string) {
  return (recentAiTexts.get(roomId) ?? []).some(
    (previous) => normalize(previous) === normalize(text) || similarity(previous, text) >= 0.72,
  );
}
function uniqueAiText(
  roomId: string,
  memberIndex: number,
  context: string[],
  fallbackPool: string[],
) {
  const member = aiMembers[memberIndex % aiMembers.length]!;
  const attempts = 8;
  for (let attempt = 0; attempt < attempts; attempt++) {
    const generated =
      buildAiConversation(member, [
        ...context,
        ...fallbackPool.slice(attempt % fallbackPool.length, (attempt % fallbackPool.length) + 1),
      ]).text || fallbackPool[(attempt + memberIndex) % fallbackPool.length]!;
    if (!isTooSimilar(roomId, generated)) {
      rememberAiText(roomId, generated);
      return generated;
    }
  }
  const fallback =
    fallbackPool.find((text) => !isTooSimilar(roomId, text)) ??
    `${member.name}: ${fallbackPool[(memberIndex + Math.floor(Math.random() * fallbackPool.length)) % fallbackPool.length]!}`;
  rememberAiText(roomId, fallback);
  return fallback;
}
...
export function getActiveDemoMembers(limit = 48): Profile[] {
  ...
  return DEMO_PROFILES.slice(0, roomCount).map((profile) => ({
    ...profile,
    status: "online",
...
export function buildDemoEntryMessages(roomId: string): MessageWithAuthor[] {
  const now = Date.now();
  const pool = roomPools[roomKey(roomId)] ?? roomPools["general"]!;
  return [0, 1, 2].map((index) => {
    const { member, author } = aiAuthor(roomId, now + index);
    const seed = pool[(now + index) % pool.length]!;
    const text =
      index === 0
        ? greetings[(now + index) % greetings.length]!
        : uniqueAiText(roomId, aiMembers.indexOf(member), [seed], pool);
...
export function buildDemoReply(roomId: string, realMessage: string): MessageWithAuthor | null {
  const now = Date.now();
  if (now - (recentlyReplied.get(roomId) ?? 0) < 4_000) return null;
  ...
  const generated = isGreeting
    ? uniqueAiText(roomId, aiMembers.indexOf(member), [realMessage], greetings)
    : uniqueAiText(roomId, aiMembers.indexOf(member), [realMessage], [...pool, ...followUps]);
  return makeMessage(roomId, author, generated, now + 1600, "ai-live");
}
...
export function buildDemoAmbientMessage(roomId: string): MessageWithAuthor | null {
  ...
  if (now - lastAiAt < 6_000) return null;
  if (now - (recentlyAmbient.get(roomId) ?? 0) < 6_000) return null;
  ...
  const previous = JSON.parse(
    window.localStorage.getItem(`diwan:ai-context:${roomId}`) || "[]",
  ) as string[];
  const pool = roomPools[roomKey(roomId)] ?? roomPools["general"]!;
  const text = uniqueAiText(roomId, aiMembers.indexOf(member), previous, pool);
  const context = [...previous, text].slice(-8);
  window.localStorage.setItem(`diwan:ai-context:${roomId}`, JSON.stringify(context));
  return makeMessage(roomId, author, text, now + 900, "ai-ambient");
}
