    ...
    <header className="glass-strong sticky top-0 z-40 border-b bg-background/70 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-2.5">
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          <Button variant="ghost" size="icon" onClick={toggleTheme} aria-label={t.common.theme}>{theme === "dark" ? <Sun className="size-5" /> : <Moon className="size-5" />}</Button>
          {isAuthenticated ? (
            <DropdownMenu>
            ...
            <div className="flex items-center gap-2"><Button variant="ghost" size="sm" asChild><Link to="/login">{t.nav.login}</Link></Button><Button size="sm" className="rounded-full px-4" asChild><Link to="/register">{t.nav.register}</Link></Button></div>
          )}
        </div>
      </div>
    </header>
