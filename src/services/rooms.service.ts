...
function isAllowedCountry(countryCode?: string | null) {
  return ALLOWED_COUNTRIES.has(normalizeCountry(countryCode));
}
...
function normalizeRoom(room: RoomDb): CommunityRoom {
  ...
  return {
    ...room,
    region: "arab",
    country: countryCode ?? "غير مصنفة",
    city: room.city_name ?? null,
    last_activity_at: room.created_at ?? null,
  } as CommunityRoom;
}
...
export async function fetchRooms(): Promise<Room[]> {
  ...
  return (data ?? [])
    .filter((room) => !room.is_private && isAllowedCountry((room as RoomDb).country_code))
    .map((room) => normalizeRoom(room as RoomDb)) as Room[];
}
...
export async function fetchRoomsWithStats(): Promise<RoomWithStats[]> {
  ...
  for (const row of messages ?? []) {
    ...
    if (!current || new Date(row.created_at).getTime() > new Date(current).getTime()) {
      lastMessages.set(row.room_id, row.created_at);
    }
  }
  ...
  return rooms.map((rawRoom) => {
    ...
    return {
      ...
      message_count: messageCounts.get(room.id) ?? 0,
      last_message_at: lastMessages.get(room.id) ?? null,
    } as RoomWithStats;
  });
}
...
export function buildCommunityTree(rooms: RoomWithStats[]): RegionNode[] {
  const scoped = rooms.filter(
    (room) => !room.is_private && isAllowedCountry((room as CommunityRoom).country),
  );
  ...
  for (const room of scoped) {
    ...
    const node = byCountry.get(country) ?? {
      country,
      cities: [],
      member_count: 0,
      message_count: 0,
  ...
  const countries = [...byCountry.values()].sort(
    (a, b) => b.message_count - a.message_count || a.country.localeCompare(b.country, "ar"),
  );
  ...
  return [
    {
      ...
      member_count: countries.reduce((sum, country) => sum + country.member_count, 0),
    },
  ];
}
...
export async function createRoom(input: {
  ...
  await supabase
    .from("room_members")
    .insert({ room_id: data.id, user_id: input.ownerId, role: "owner" });
...
export const roomsWithStatsQuery = () =>
  queryOptions({
    queryKey: ["rooms", "stats", "all-public"],
    queryFn: fetchRoomsWithStats,
    staleTime: 30_000,
  });
