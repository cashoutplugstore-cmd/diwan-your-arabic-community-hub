...
export type PremiumSubscription = { id: string; user_id: string; plan: PremiumPlan; status: "pending" | "active" | "expired" | "cancelled"; started_at: string | null; expires_at: string | null; created_at: string };
...
export async function fetchMyPremiumSubscription(userId: string) {
  const { data, error } = await looseDb.from("premium_subscriptions").select("id,user_id,plan,status,started_at,expires_at,created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(1).maybeSingle();
...
export async function startPremiumCheckout(userId: string, plan: PremiumPlan, email?: string | null) {
  ...
  const { data, error } = await supabase.functions.invoke("stripe-create-checkout-v2", { body: { priceId: PRICE_IDS[plan], userId, email: email ?? undefined } });
  if (error) throw error;
