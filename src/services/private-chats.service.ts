...
export async function fetchMyPrivateChats(userId: string): Promise<PrivateChat[]> {
  ...
  const [
    ...
    supabase
      ...
      .in("room_id", ids)
      .eq("is_deleted", false),
    (supabase as any).rpc("private_chat_unread_counts") as Promise<{
      data: UnreadRow[] | null;
      error: any;
    }>,
  ]);
  ...
  for (const row of members ?? [])
    if (row.user_id && row.user_id !== userId) otherIds.add(row.user_id);
  for (const room of rooms)
    if (room.owner_id && room.owner_id !== userId) otherIds.add(room.owner_id);
  ...
  for (const row of members ?? []) {
    if (row.room_id && row.user_id && row.user_id !== userId && !memberByRoom.has(row.room_id))
      memberByRoom.set(row.room_id, row.user_id);
  }
  for (const room of rooms) {
    if (!memberByRoom.has(room.id) && room.owner_id && room.owner_id !== userId)
      memberByRoom.set(room.id, room.owner_id);
  }
  ...
  for (const row of messages ?? []) {
    ...
    if (!current || new Date(row.created_at).getTime() > new Date(current).getTime())
      lastByRoom.set(row.room_id, row.created_at);
  }
  const unreadByRoom = new Map(
    (unreadRows ?? []).map((row) => [row.room_id, Number(row.unread_count) || 0]),
  );
...
export const myPrivateChatsQuery = (userId: string | undefined) =>
  queryOptions({
...
export const myPrivateChatUnreadCountQuery = (userId: string | undefined) =>
  queryOptions({
    queryKey: ["private-chat-unread-count", userId],
    queryFn: async () => {
      const { data, error } = (await (supabase as any).rpc("private_chat_unread_counts")) as {
        data: UnreadRow[] | null;
        error: any;
      };
