 ...
 */
export async function triggerAIRoomReplies(input: {
  roomId: string;
  messageId: string;
  message?: string;
  createdAt?: string;
  onReply?: AIReplyHandler;
}) {
...
export async function sendMessage(input: {
  ...
  if (input.onAIReply) {
    void triggerAIRoomReplies({
      roomId: input.roomId,
      messageId: data.id,
      message: content,
      createdAt: data.created_at ?? clientCreatedAt,
      onReply: input.onAIReply,
    });
  }
}
