...
export const EMPTY_PERMISSIONS: RoomPermissions = {
  role: null, isGlobalOwner: false, isOwner: false, isGlobalAdmin: false, isRoomAdmin: false, isRoomModerator: false,
  canManageRoles: false, canGrantRoomAdmin: false, canModerate: false, canMute: false, canBan: false, canDeleteMessages: false,
};

export async function fetchRoomPermissions(roomId: string, userId: string): Promise<RoomPermissions> {
  const [{ data: room }, { data: membership }, { data: globalRoles }, globalOwner] = await Promise.all([
  ...
  const canManageRoles = globalOwner || isOwner || isGlobalAdmin || isRoomAdmin;
  const canGrantRoomAdmin = globalOwner || isOwner || isGlobalAdmin;
  const canModerate = globalOwner || canManageRoles || isGlobalModerator;
  return {
    role: globalOwner || isOwner ? "owner" : memberRole,
...
export const roomPermissionsQuery = (roomId: string | undefined, userId: string | undefined) =>
  queryOptions({ queryKey: ["room-permissions", roomId, userId], queryFn: () => fetchRoomPermissions(roomId!, userId!), enabled: Boolean(roomId && userId), staleTime: 30_000 });
...
export const roomMemberRolesQuery = (roomId: string | undefined) => queryOptions({ queryKey: ["room-member-roles", roomId], queryFn: () => fetchRoomMemberRoles(roomId!), enabled: Boolean(roomId) });
