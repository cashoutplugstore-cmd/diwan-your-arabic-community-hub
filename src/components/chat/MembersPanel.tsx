...
export function MembersPanel(props: Props) {
  ...
  if (isMobile) return <>{!open && props.roomId ? <button type="button" onClick={() => setOpen(true)} className="fixed bottom-[5.75rem] end-3 z-[65] inline-flex h-10 items-center gap-1.5 rounded-full border bg-background/95 px-3 text-xs font-bold shadow-xl backdrop-blur" aria-label="فتح أعضاء الغرفة وتعديل الرتب"><Settings2 className="size-4" />الرتب</button> : null}{open ? <div className="fixed inset-0 z-[70]" role="presentation"><button type="button" className="absolute inset-0 bg-background/60 backdrop-blur-sm" onClick={() => setOpen(false)} aria-label="إغلاق الأعضاء" /><aside className="absolute inset-y-0 end-0 flex w-[min(88vw,22rem)] flex-col overflow-hidden border-s bg-background shadow-2xl" role="dialog" aria-modal="true" aria-label={t.chat.members}><div className="flex shrink-0 items-center justify-between border-b p-2"><span className="px-2 text-xs font-semibold text-muted-foreground">{t.chat.members}</span><Button type="button" variant="ghost" size="icon" onClick={() => setOpen(false)} aria-label="إغلاق"><X className="size-4" /></Button></div><PanelContent {...props} /></aside></div> : null}</>;
