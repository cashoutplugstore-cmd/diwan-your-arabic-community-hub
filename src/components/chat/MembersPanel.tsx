...
import { Crown, Mic2, Users, LogIn, LogOut, Shield, ShieldCheck, X } from "lucide-react";
...
type Role = "admin" | "moderator" | "vip" | "speaker" | null;
...
function PanelContent({ members, presence, activity = [], roomId }: Props) {
  ...
  const roleById = useMemo(() => {
    ...
    for (const row of rolesQuery.data?.roles ?? []) {
      ...
      else if (row.role === "moderator" && map.get(row.user_id) !== "admin") map.set(row.user_id, "moderator");
    }
  ...
  const staff = allRows.filter((row) => row.role === "admin" || row.role === "moderator");
  ...
  const renderRow = (row: Row) => (
    <li key={row.id} className="group flex min-w-0 items-center gap-2 rounded-xl px-2 py-2 transition-colors hover:bg-secondary/60">
      <UserAvatar name={row.name} src={row.avatar} size="sm" status={row.status} role={row.role} />
      <span className={"min-w-0 flex-1 truncate text-xs font-medium sm:text-sm " + (row.role === "vip" ? "text-fuchsia-300" : row.role === "admin" ? "text-amber-300" : row.role === "moderator" ? "text-sky-300" : "")}>{row.name}</span>
      {row.role === "admin" ? <ShieldCheck className="size-3 shrink-0 text-amber-300" aria-label="ADMIN" /> : null}
      {row.role === "moderator" ? <Shield className="size-3 shrink-0 text-sky-300" aria-label="مشرف" /> : null}
  ...
  return <>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto scrollbar-slim pb-3">{section("👑 الإدارة والمشرفون", staff)}{section("🎙️ على المايك", speakers)}{section("💎 VIP", vip)}{section("المتواجدون", online)}{allRows.length === 0 ? <p className="p-4 text-center text-xs text-muted-foreground">لا يوجد أعضاء متواجدون حالياً.</p> : null}</div>
