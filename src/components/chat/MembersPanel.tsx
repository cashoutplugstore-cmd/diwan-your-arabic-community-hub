...
function PanelContent({ members, presence, activity = [] }: Props) {
  ...
  return <>
    <header className="flex items-center gap-2 border-b px-3 py-3 sm:px-4 sm:py-4"><Users className="size-4 shrink-0 text-primary sm:size-5" aria-hidden /><h2 className="min-w-0 truncate font-display text-xs font-bold sm:text-sm">{t.chat.members}</h2><Badge variant="secondary" className="ms-auto shrink-0 px-1.5 text-[10px] sm:px-2 sm:text-xs">{onlineCount}</Badge></header>
    <div className="flex items-center justify-between gap-1 border-b px-3 py-2 text-[9px] text-muted-foreground sm:px-4 sm:py-2.5 sm:text-xs"><span>{rows.length} أعضاء</span><span className="flex items-center gap-1"><span className="size-1.5 rounded-full bg-emerald-400" />نشط</span></div>
    <ul className="min-h-0 flex-1 space-y-1 overflow-y-auto scrollbar-slim p-2.5">{rows.slice(0, 48).map((row, index) => <li key={row.id} className="group flex min-w-0 items-center gap-2 rounded-xl px-2 py-2 transition-colors hover:bg-secondary/60"><div className="relative shrink-0"><UserAvatar name={row.name} src={row.avatar} size="sm" status={row.status} />{index === 0 ? <span className="absolute -start-1 -top-1 grid size-4 place-items-center rounded-full bg-amber-400 text-black"><Crown className="size-2.5" /></span> : null}</div><span className={`min-w-0 flex-1 truncate text-xs font-medium sm:text-sm ${colorFor(row.id)}`} title={row.name || "—"}>{row.name || "—"}</span>{row.status === "online" && index % 7 === 0 ? <Mic2 className="size-3 shrink-0 text-emerald-400" aria-label="يتحدث" /> : null}</li>)}</ul>
