...
function PanelContent({ members, presence, activity = [], roomId }: Props) {
  ...
  const rolesQuery = useQuery({
    queryKey: ["member-roles", memberIds.join(",")],
    ...
    queryFn: async () => {
      ...
      return {
        roles: roles ?? [],
        vip: (vip ?? []).filter((row) => !row.expires_at || new Date(row.expires_at).getTime() > Date.now()),
      };
    },
    staleTime: 30_000,
  });
  ...
  const makeRole = (id: string): Role => speakerById.has(id) ? "speaker" : (roleById.get(id) ?? null);
  const rows = useMemo<Row[]>(() => members.map((member) => {
    const p = presenceById.get(member.id);
    return { id: member.id, name: member.display_name || member.username || "—", avatar: member.avatar_url, status: p?.status ?? "offline", role: makeRole(member.id), speaking: speakerById.has(member.id) };
  }), [members, presence, roleById, speakerById]);
  const extraRows = presence.filter((entry) => !members.some((member) => member.id === entry.userId)).map<Row>((entry) => ({ id: entry.userId, name: entry.displayName, avatar: entry.avatarUrl, status: entry.status, role: makeRole(entry.userId), speaking: speakerById.has(entry.userId) }));
  const allRows = [...rows, ...extraRows];
  const staff = allRows.filter((row) => row.role === "admin" || row.role === "owner" || row.role === "moderator");
  const speakers = allRows.filter((row) => row.speaking && !staff.some((s) => s.id === row.id));
  const vip = allRows.filter((row) => row.role === "vip");
  const online = allRows.filter((row) => row.status !== "offline" && !speakers.some((s) => s.id === row.id) && !staff.some((s) => s.id === row.id) && !vip.some((v) => v.id === row.id));
  ...
  const renderRoleActions = (row: Row) => {
    ...
    return <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button type="button" variant="ghost" size="icon" className="size-7 shrink-0 opacity-0 transition-opacity group-hover:opacity-100 focus:opacity-100" aria-label={`تعديل رتبة ${row.name}`}>
          <MoreVertical className="size-4" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="min-w-44">
        <DropdownMenuItem onClick={() => changeRole.mutate({ userId: row.id, role: isModerator ? "member" : "moderator" })} disabled={changeRole.isPending}>
          <Shield className="size-4" />
          {isModerator ? "إزالة رتبة المشرف" : "تعيين كمشرف"}
        </DropdownMenuItem>
        {isModerator ? <DropdownMenuSeparator /> : null}
        {isModerator ? <DropdownMenuItem onClick={() => changeRole.mutate({ userId: row.id, role: "member" })} disabled={changeRole.isPending}>إرجاع إلى عضو</DropdownMenuItem> : null}
      </DropdownMenuContent>
    </DropdownMenu>;
  };

  const renderRow = (row: Row) => <li key={row.id} className={"group relative flex min-w-0 items-center gap-2 rounded-xl px-2 py-2 transition-all hover:bg-secondary/60 " + (row.role === "admin" ? "border border-rose-400/30 bg-gradient-to-r from-rose-500/[0.10] to-pink-500/[0.04] shadow-[0_0_18px_rgba(244,63,94,.10)]" : row.role === "owner" ? "border border-amber-400/20 bg-amber-500/[0.05]" : row.role === "moderator" ? "border border-sky-400/15 bg-sky-500/[0.04]" : "")}>
    {row.role === "admin" ? <span className="absolute -start-0.5 top-1/2 h-8 w-0.5 -translate-y-1/2 rounded-full bg-gradient-to-b from-rose-300 via-pink-500 to-red-500" /> : null}
    <UserAvatar name={row.name} src={row.avatar} size="sm" status={row.status} role={row.role === "owner" ? "admin" : row.role} />
    <span className={"min-w-0 flex-1 truncate text-xs sm:text-sm " + (row.role === "admin" ? "font-black text-rose-300 drop-shadow-[0_0_8px_rgba(244,63,94,.35)]" : row.role === "owner" ? "font-black text-amber-300" : row.role === "vip" ? "font-bold text-fuchsia-300" : row.role === "moderator" ? "font-bold text-sky-300" : "font-semibold")}>{row.role === "admin" ? "🌹 👑 " : row.role === "owner" ? "👑 " : ""}{row.name}</span>
