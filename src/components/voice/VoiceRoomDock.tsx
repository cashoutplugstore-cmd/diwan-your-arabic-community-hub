...
export function VoiceRoomDock({ roomName }: { roomName?: string }) {
  ...
  useEffect(() => {
    ...
    async function loadParticipants() {
      ...
      const { data: profiles } = await supabase
        ...
        .in("id", ids);
      const names = new Map(
        (profiles ?? []).map((p) => [p.id, p.display_name || p.username || "عضو"]),
      );
      if (active)
        setParticipants(
          (data ?? []).map((row) => ({ ...row, display_name: names.get(row.user_id) ?? "عضو" })),
        );
    }
  ...
  async function joinVoice() {
    ...
    const { error } = await supabase.from("room_voice_participants").insert({
      room_id: roomId,
      user_id: user.id,
      is_speaker: true,
      is_muted: false,
  ...
  async function setSelfMuted(muted: boolean) {
    if (!roomId) return;
    const { error } = await supabase.rpc("voice_set_self_muted", {
      _room_id: roomId,
      _muted: muted,
    });
  ...
  return (
    <div className="border-t bg-background/80 px-3 py-2 backdrop-blur-sm" aria-label="الصوت">
      {participants.length > 0 ? (
        <div className="mb-2 flex items-center gap-2 overflow-x-auto rounded-xl border border-emerald-400/20 bg-emerald-500/5 px-2 py-1.5">
          ...
          {participants.map((p) => (
            <span
              key={p.user_id}
              className="inline-flex shrink-0 items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-semibold text-emerald-200"
            >
      ...
      <div className="flex items-center gap-2">
        <div className="hidden min-w-0 flex-1 sm:block">
          ...
          <p className="truncate text-[10px] text-muted-foreground">
            {micOn ? "أنت صاعد على المايك" : "ارفع المايك وتكلم مع الموجودين"}
          </p>
        </div>
        <Button
          type="button"
        ...
        >
          ...
          <span className="truncate">{micOn ? "🎙️ على المايك" : "صعود المايك"}</span>
        </Button>
        {micOn ? (
          <Button
            type="button"
            ...
            title={speakerOn ? "كتم المايك" : "إلغاء كتم المايك"}
            aria-label={speakerOn ? "كتم المايك" : "إلغاء كتم المايك"}
          >
            {speakerOn ? <Volume2 className="size-4" /> : <MicOff className="size-4" />}
          </Button>
        ) : (
          <Button
            type="button"
        ...
        >
          ...
          <input
            ...
            type="file"
            accept="audio/*"
            onChange={(event) => chooseMusic(event.target.files?.[0])}
          />
        </label>
        {micOn ? (
          <Button
      ...
      {mediaUrl ? (
        <div className="mt-2 flex min-w-0 items-center gap-2 rounded-xl bg-primary/10 px-2.5 py-1.5 text-[11px]">
          <audio
            ref={audioRef}
            src={mediaUrl}
