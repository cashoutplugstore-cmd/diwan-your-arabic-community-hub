...
type Role = "global_admin" | "owner" | "admin" | "moderator" | "vip" | "speaker";
...
const icons = { global_admin: Gem, owner: Crown, admin: ShieldCheck, moderator: Shield, vip: Crown, speaker: Mic2 } as const;
...
export function UserAvatar({ name, src, size = "md", status, role, className, showMemberBadge = false, autoCurrentRole = true }: Props) {
  ...
  const effectiveRole: Role | null = role ?? (isCurrentUser && currentRoles.data?.isAdmin ? "global_admin" : isCurrentUser && currentRoles.data?.isModerator ? "moderator" : isCurrentUser && premium.data ? "vip" : null);
