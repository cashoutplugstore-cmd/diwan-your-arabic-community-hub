...
type Props = { name?: string | null; src?: string | null; size?: "sm" | "md" | "lg"; status?: string | null; role?: Role | null; className?: string; showMemberBadge?: boolean; autoCurrentRole?: boolean };
...
export function UserAvatar({ name, src, size = "md", status, role, className, showMemberBadge = false, autoCurrentRole = true }: Props) {
  ...
  const publicStaff = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        profiles: (profiles ?? []) as PublicProfile[],
        roles: (roles ?? []) as PublicRole[],
      };
    },
  });
  ...
  // Compact avatars are used inside chat messages and the members list. Their role is
  // already shown next to the name, so rendering another pill above the avatar creates
  // a duplicated "ADMIN/MOD/عضو" control and pushes the avatar/message out of alignment.
  ...
  return (
    <div className={cn("relative shrink-0", showCompactRoleBadge && "pt-2", className)}>
      ...
      {effectiveRole === "admin" ? <Crown className="absolute -end-1 -top-1 size-3 text-sky-300 drop-shadow" /> : null}
