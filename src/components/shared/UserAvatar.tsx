...
export function UserAvatar({ name, src, size = "md", status, role, className, showMemberBadge = false, autoCurrentRole = true }: Props) {
  ...
  const matchedPublicProfile = (() => { if (!name || role || !publicStaff.data) return null; const candidates = publicStaff.data.profiles.filter((p) => p.username === name || p.display_name === name); return candidates.length === 1 ? candidates[0] : candidates.find((p) => Boolean(src && p.avatar_url === src)) ?? null; })();
  ...
  const openProfile = () => { if (profileTarget) void navigate({ to: "/profile/$userId", params: { userId: profileTarget } }); };
