...
function parseMedia(content: string): MediaPayload | null {
  try {
    ...
    return value?.type && value?.path ? (value as MediaPayload) : null;
  } catch {
    return null;
  }
}
...
export function PrivateChatPage({ slug }: { slug: string }) {
  ...
  const messageUserIds = useMemo(
    () => [...new Set(items.map((m) => m.user_id).filter(Boolean))],
    [items],
  );
  ...
  useEffect(() => {
    ...
    const loadRoles = async () => {
      ...
      for (const row of data ?? []) {
        ...
        else if (row.role === "moderator" && next[row.user_id] !== "admin")
          next[row.user_id] = "moderator";
      }
    ...
    const load = async () => {
      const entries = await Promise.all(
        items.map(async (m) => {
          ...
          try {
            return [m.id, await signedMediaUrl(media.path)] as const;
          } catch {
            return null;
          }
        }),
  ...
  const sendText = useMutation({
    mutationFn: (content: string) => sendMessage({ roomId: roomId!, userId: user!.id, content }),
    onSuccess: () => {
      setDraft("");
  ...
  async function uploadBlob(blob: Blob, type: "image" | "audio", name: string) {
    ...
    try {
      ...
      await sendMessage({
        roomId,
        userId: user.id,
        content: JSON.stringify({ type, path, name, mime: blob.type, size: blob.size }),
      });
      await qc.invalidateQueries({ queryKey: ["messages", roomId] });
    } catch (error) {
      toast.error((error as Error).message || "تعذر إرسال الملف");
    } finally {
      setUploading(false);
    }
  }
  ...
  async function toggleRecording() {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === "undefined") {
      toast.error("تسجيل الصوت غير مدعوم على هذا الجهاز");
      return;
    }
    try {
      ...
      recorder.onstop = () => {
        ...
        void uploadBlob(blob, "audio", `voice-${Date.now()}.webm`);
        setRecording(false);
      };
      recorder.start();
      ...
      toast.error("اسمح للتطبيق باستخدام المايك لتسجيل رسالة صوتية");
    }
  }

  function submit() {
    const content = draft.trim();
    if (!content || !roomId || !user || sendText.isPending) return;
    sendText.mutate(content.slice(0, MAX_MESSAGE_LENGTH));
  }

  if (room.isLoading) return <MessagesSkeleton />;
  if (!room.data || room.data.is_private !== true)
    return (
      <EmptyState title="هذه ليست محادثة خاصة" description="افتح المحادثة من بروفايل العضو." />
    );

  return (
    <section className="glass-strong flex min-h-0 h-[calc(100dvh-115px)] flex-1 flex-col overflow-hidden rounded-2xl lg:h-[calc(100dvh-175px)] lg:rounded-3xl">
      <header className="flex shrink-0 items-center gap-3 border-b bg-background/35 px-3 py-3 sm:px-4">
        ...
        <Link
          to="/profile/$userId"
          params={{ userId: room.data.owner_id }}
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[10px] text-muted-foreground sm:text-xs">
            محادثة خاصة • بينك وبين العضو فقط
          </p>
        </div>
      </header>
      <div className="relative min-h-0 flex-1 overflow-y-auto px-3 py-4 scrollbar-slim">
        <div className="mx-auto w-full max-w-3xl space-y-2">
          {messages.hasNextPage ? (
            <div className="flex justify-center">
              <Button variant="ghost" size="sm" onClick={() => void messages.fetchNextPage()}>
                تحميل الأقدم
              </Button>
            </div>
          ) : null}
          {messages.isLoading ? (
            ...
            <EmptyState title="لا توجد رسائل" description="ابدأ المحادثة بإرسال رسالة." />
          ) : (
            items.map((message) => {
              const mine = message.user_id === user?.id;
              const media = parseMedia(message.content);
              const url = mediaUrls[message.id];
              const role = roleMap[message.user_id];
              ...
              return (
                ...
                >
                  <Link
                    ...
                    className="shrink-0 rounded-xl"
                    aria-label={`فتح بروفايل ${authorName}`}
                  >
                    <UserAvatar
                      name={authorName}
                  ...
                  <div
                    className={`max-w-[82%] rounded-2xl px-3 py-2 ${mine ? "bg-primary text-primary-foreground rounded-te-md" : "bg-secondary/80 rounded-ts-md"}`}
                  >
                    ...
                    {media?.type === "image" ? (
                      url ? (
                        ...
                        <span className="text-xs opacity-70">جاري تحميل الصورة…</span>
                      )
                    ) : media?.type === "audio" ? (
                      url ? (
      ...
      >
        ...
        <div className="mx-auto flex max-w-3xl items-end gap-1.5 rounded-2xl border bg-secondary/45 p-1.5 shadow-sm focus-within:border-primary/40">
          <Button
            ...
            disabled={uploading || sendText.isPending}
          ...
          <Button
            ...
            size="icon"
            className="size-10 shrink-0 rounded-xl"
            disabled={!draft.trim() || sendText.isPending || uploading || !user}
            aria-label="إرسال"
          >
            <SendHorizonal className="size-4" />
          </Button>
        </div>
        <p className="mx-auto mt-1 max-w-3xl px-2 text-[9px] text-muted-foreground">
          صور حتى 8MB • التسجيل الصوتي من المايك
        </p>
      </form>
    </section>
  );
