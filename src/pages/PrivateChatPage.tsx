...
import { ArrowLeft, ImagePlus, Mic, Paperclip, SendHorizonal, Square, Volume2 } from "lucide-react";
...
export function PrivateChatPage({ slug }: { slug: string }) {
  ...
  const sendText = useMutation({
    ...
    onSuccess: () => { setDraft(""); void qc.invalidateQueries({ queryKey: ["messages", roomId] }); },
  ...
  async function uploadBlob(blob: Blob, type: "image" | "audio", name: string) {
    ...
    } catch (error) { toast.error((error as Error).message || "تعذر إرسال الملف"); }
  ...
  return <section className="glass-strong flex min-h-0 h-[calc(100dvh-115px)] flex-1 flex-col overflow-hidden rounded-2xl lg:h-[calc(100dvh-175px)] lg:rounded-3xl">
    <header className="flex shrink-0 items-center gap-3 border-b bg-background/35 px-3 py-3 sm:px-4">
      ...
      <div className="min-w-0 flex-1"><h1 className="truncate font-display text-sm font-black sm:text-base">{title}</h1><p className="text-[10px] text-muted-foreground sm:text-xs">محادثة خاصة • بينك وبين العضو فقط</p></div>
    </header>
