...
const monthlyFeatures = [...vipFeatures, "مزايا اجتماعية متقدمة", "أولوية في المزايا الجديدة"];
const fourMonthFeatures = [...monthlyFeatures, "قيمة أفضل لمدة 4 أشهر"];
const yearlyFeatures = [...fourMonthFeatures, "أفضل قيمة سنوية"];
...
export function PremiumPage() {
  ...
  const checkout = useMutation({
    mutationFn: (plan: PremiumPlan) => startPremiumCheckout(user!.id, plan, user!.email),
    ...
    <div className="diwan-vip-page mx-auto max-w-7xl space-y-6 py-2">
      <section className="diwan-vip-hero glass-strong relative overflow-hidden rounded-3xl p-6 sm:p-10">
        ...
        <div className="relative z-10 max-w-4xl">
          <Badge className="mb-4 gap-1 rounded-full px-3 py-1">
            <Crown className="size-3.5" /> ديوان Premium
          </Badge>
          <h1 className="font-display text-3xl font-black tracking-tight sm:text-5xl">
            اختار اشتراكك وخلي ديوان نار 🔥
          </h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-muted-foreground sm:text-lg">
            دفع آمن عبر Stripe، وتفعيل الاشتراك تلقائياً بعد نجاح الدفع.
          </p>
        </div>
      </section>
      {active ? (
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                أنت الآن ضمن أعضاء ديوان Premium.
              </p>
            </div>
          </div>
        </section>
      ...
      <div className="grid gap-5 xl:grid-cols-4 md:grid-cols-2">
        <PlanCard
          ...
          disabled={pending || active || checkout.isPending}
      ...
      <section className="grid gap-5 md:grid-cols-2">
        <FeatureHighlight
          ...
          title="هوية Premium"
          text="شارة وتأثيرات وثيمات حصرية للبروفايل."
        />
        <FeatureHighlight
          icon={<EyeOff className="size-5" />}
          title="خصوصية متقدمة"
          text="تحكم أكبر في Online وRead receipts."
        />
        <FeatureHighlight
          icon={<UsersRound className="size-5" />}
          title="مزايا المجتمع"
          text="مزايا مخصصة للمشتركين والغرف والمجتمعات."
        />
        <FeatureHighlight
          icon={<Globe2 className="size-5" />}
          title="دفع آمن"
          text="تتم عملية الدفع في Stripe ولا نخزن بيانات البطاقة داخل ديوان."
        />
      </section>
    </div>
  );
...
function PlanCard({
  ...
  return (
    <article
      className={`diwan-vip-card glass flex h-full flex-col rounded-3xl p-6 ${featured ? "diwan-vip-card-featured" : ""}`}
    >
      ...
      >
        ...
        {disabled ? "قيد المعالجة" : "اشترك الآن"}
      </Button>
    </article>
  );
}
