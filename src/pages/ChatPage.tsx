  ...
  return parts.map((part, index) => {
    ...
    return (
      <span
        ...
        className={
          mine
            ? "rounded bg-primary/25 px-1 font-medium text-primary"
            : "rounded bg-secondary px-1 font-medium"
        }
      >
        {part}
      </span>
...
export function ChatRoomPage({ slug }: { slug: string }) {
  ...
  const messages = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage) =>
      lastPage.length < MESSAGE_PAGE_SIZE ? undefined : (lastPage[0]?.created_at ?? undefined),
  });
  ...
  const canModerate =
    Boolean(roles.data?.isStaff) || (room.data?.owner_id != null && room.data.owner_id === user?.id);
  ...
  const appendIncoming = useCallback(
    async (message: Message) => {
      ...
      queryClient.setQueryData<{ pages: MessageWithAuthor[][]; pageParams: unknown[] }>(
        ["messages", roomId],
        (prev) => {
          if (!prev) return prev;
      ...
      if (message.user_id !== user?.id) {
        const mentioned =
          ...
          message.content.toLowerCase().includes(`@${profile.data.username.toLowerCase()}`);
          ...
          prev
            ? {
                ...
                pages: prev.pages.map((page) =>
                  page.map((m) => (m.id === message.id ? { ...m, ...message } : m)),
                ),
              }
            : prev,
      );
  ...
  // Smart auto-scroll: only follow new messages when already near the bottom.
  useEffect(() => {
    if (!atBottom) return;
  ...
  const send = useMutation({
    mutationFn: (content: string) =>
      sendMessage({ roomId: roomId!, userId: user!.id, content, replyToId: replyTo?.id ?? null }),
  ...
  const block = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["user_blocks"] });
    },
    onError: (error: Error) => toast.error(error.message),
  });

  const restrict = useMutation({
    mutationFn: ({ targetId, kind }: { targetId: string; kind: "ban" | "mute" }) =>
      restrictInRoom({ roomId: roomId!, userId: targetId, kind, createdBy: user!.id }),
    onSuccess: (_data, variables) => {
      toast.success(variables.kind === "ban" ? t.moderation.banned : t.moderation.muted);
      void queryClient.invalidateQueries({ queryKey: ["room_moderation", roomId] });
    },
    onError: (error: Error) => toast.error(error.message),
  });
    ...
    <div className="flex gap-4">
      <div className="glass-strong flex h-[calc(100dvh-190px)] min-w-0 flex-1 flex-col overflow-hidden rounded-3xl lg:h-[calc(100dvh-140px)]">
        <header className="flex min-w-0 items-center gap-3 border-b px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs text-muted-foreground">
              ...
              {room.data.country ? ` · ${room.data.country}` : ""}
            </p>
          </div>
        ...
        <div
          ref={scrollRef}
          onScroll={onScroll}
          className="relative flex-1 space-y-3 overflow-y-auto scrollbar-slim p-4"
        >
          {messages.hasNextPage ? (
            <div className="flex justify-center">
              ...
              >
                ...
                {messages.isFetchingNextPage ? t.common.loading : t.chat.loadMore}
              </Button>
            </div>
          ) : null}
                  ...
                  <div className={mine ? "group flex flex-row-reverse gap-3" : "group flex gap-3"}>
                    ...
                    <div className="min-w-0 max-w-[78%]">
                      <div
                        className={
                          mine
                            ? "rounded-2xl rounded-te-sm bg-primary px-4 py-2.5 text-primary-foreground"
                            : "rounded-2xl rounded-ts-sm bg-secondary px-4 py-2.5 text-secondary-foreground"
                        }
                      >
                        <p className="text-xs opacity-70">{authorName}</p>
                        {parent ? (
                          <p className="mt-1 truncate border-s-2 border-current/40 ps-2 text-[11px] opacity-70">
                            {parent.author?.display_name || parent.author?.username || "—"}:{" "}
                            {parent.is_deleted ? t.chat.deleted : parent.content}
                          </p>
                        ) : null}
                        {message.is_deleted ? (
                          <p className="text-sm italic opacity-60">{t.chat.deleted}</p>
                          ...
                          <div className="mt-1 space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Button
                                ...
                                disabled={saveEdit.isPending || !editing.content.trim()}
                              >
                                <Check className="size-3.5" aria-hidden /> {t.common.save}
                              </Button>
                          ...
                          <p className="whitespace-pre-wrap break-words text-sm">
                            {highlight(message.content, profile.data?.username)}
                          </p>
                        )}
                        <p className="mt-1 flex items-center gap-2 text-[10px] opacity-60">
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            ...
                            className="size-8 self-center opacity-0 transition-opacity focus-visible:opacity-100 group-hover:opacity-100 motion-reduce:transition-none"
                          >
                        ...
                        <DropdownMenuContent align="end" className="w-52">
                          <DropdownMenuItem onClick={() => setReplyTo(message)}>
                            <Reply className="size-4" /> {t.chat.reply}
                          </DropdownMenuItem>
                          <DropdownMenuItem
                            onClick={() => {
                          ...
                          >
                            <Copy className="size-4" /> {t.chat.copy}
                          </DropdownMenuItem>
                          {mine ? (
                            <DropdownMenuItem onClick={() => setEditing(message)}>
                              <Pencil className="size-4" /> {t.chat.edit}
                            </DropdownMenuItem>
                          ) : null}
                          {mine || canModerate ? (
                            <DropdownMenuItem
                              ...
                              <ReportDialog
                                ...
                                trigger={
                                  <DropdownMenuItem onSelect={(event) => event.preventDefault()}>
                                    <Flag className="size-4" /> {t.moderation.report}
                                  </DropdownMenuItem>
                                }
                              />
                              <DropdownMenuItem onClick={() => block.mutate(message.user_id)}>
                                <UserX className="size-4" /> {t.moderation.block}
                              </DropdownMenuItem>
                              {canModerate ? (
                                <>
                                  <DropdownMenuItem
                                    onClick={() =>
                                      restrict.mutate({ targetId: message.user_id, kind: "mute" })
                                    }
                                  >
                                    <VolumeX className="size-4" /> {t.moderation.mute}
                                  </DropdownMenuItem>
                                  <DropdownMenuItem
                                    onClick={() =>
                                      restrict.mutate({ targetId: message.user_id, kind: "ban" })
                                    }
                                  >
                                    <Ban className="size-4" /> {t.moderation.ban}
                                  </DropdownMenuItem>
                                </>
                              ) : null}
                            </>
        ...
        {unseen > 0 && !atBottom ? (
          <div className="pointer-events-none relative">
            <Button
              ...
              className="pointer-events-auto absolute -top-14 inset-x-0 mx-auto w-fit shadow-glow"
            >
              <ArrowDown className="size-4" aria-hidden /> {unseen} {t.chat.newMessages}
            </Button>
          </div>
        ) : null}

        {replyTo ? (
          <div className="flex items-center gap-2 border-t bg-secondary/40 px-4 py-2 text-xs">
            <Reply className="size-3.5 shrink-0 text-primary" aria-hidden />
            <span className="min-w-0 flex-1 truncate">
              {t.chat.replyingTo} {replyTo.author?.display_name || replyTo.author?.username}:{" "}
              {replyTo.content}
            </span>
            <Button
              variant="ghost"
        ...
        {isBanned || isMuted ? (
          <div className="flex items-center gap-2 border-t bg-destructive/10 px-4 py-3 text-sm text-destructive">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t.chat.confirmDeleteDesc}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t.common.cancel}</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => pendingDelete && removeMessage.mutate(pendingDelete.id)}
            >
              {t.chat.delete}
            </AlertDialogAction>
          </AlertDialogFooter>
