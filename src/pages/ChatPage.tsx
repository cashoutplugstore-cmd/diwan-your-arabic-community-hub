...
import { dayKey, dayLabel, timeOfDay } from "@/lib/time";
...
export function ChatIndexPage() {
  return (
    <EmptyState
      icon={MessagesSquare}
      title="المحادثات"
      description="استخدم قائمة المحادثات الخاصة."
    />
  );
}
function highlight(content: string) {
  return content.split(/(@[\p{L}\p{N}_.-]+)/gu).map((part, index) =>
    part.startsWith("@") ? (
      <span key={index} className="rounded bg-secondary px-1 font-medium">
        {part}
      </span>
    ) : (
...
export function ChatRoomPage({ slug }: { slug: string }) {
  ...
  const memberIds = useMemo(
    () => [...(members.data ?? []).map((m) => m.id)].filter(Boolean),
    [members.data],
  );
  const memberRoles = useQuery({
    ...
    queryFn: async () => {
      const [{ data: global, error: globalError }, { data: roomRoles, error: roomError }] =
        await Promise.all([
  ...
  const roleById = useMemo(() => {
    ...
    for (const row of memberRoles.data?.room ?? []) {
      if (row.role === "moderator") map.set(row.user_id, "moderator");
    }
  ...
  const blockedIds = useMemo(
    () => new Set((blocks.data ?? []).map((b) => b.blocked_id)),
    [blocks.data],
  );
  const myRestrictions = useMemo(
    () => (restrictions.data ?? []).filter((r) => r.user_id === user?.id),
    [restrictions.data, user?.id],
  );
  ...
  const myRole = roleById.get(user?.id ?? "");
  const canModerate =
    permissions.canModerate ||
    !!roles.data?.isAdmin ||
    !!roles.data?.isModerator ||
    myRole === "admin" ||
    myRole === "moderator" ||
    room.data?.owner_id === user?.id;
  const canManageRoles =
    permissions.canManageRoles ||
    !!roles.data?.isAdmin ||
    roleById.get(user?.id ?? "") === "admin" ||
    room.data?.owner_id === user?.id;
  const items = useMemo(() => {
    const flat = [...(messages.data?.pages ?? [])].reverse().flat();
    const combined = [...flat, ...demoMessages].sort(
      (a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime(),
    );
    const seen = new Set<string>();
    const demoContentSeen = new Set<string>();
    return combined.filter((message) => {
      if (seen.has(message.id)) return false;
      if (message.user_id.startsWith("demo-")) {
        const key = message.content.trim().replace(/\s+/g, " ").toLowerCase();
        if (demoContentSeen.has(key)) return false;
        demoContentSeen.add(key);
      }
      seen.add(message.id);
  ...
  const appendIncoming = useCallback(
    async (message: Message) => {
      ...
      let author: Profile | null =
        (members.data ?? []).find((m) => m.id === message.user_id) ?? null;
      if (!author) {
        ...
        prev
          ? {
              ...
              pages: prev.pages.map((p: any) =>
                p.map((x: any) => (x.id === message.id ? { ...x, ...message } : x)),
              ),
            }
          : prev,
      );
      ...
      sendMessage({
        ...
        onAIReply: (reply) =>
          setDemoMessages((current) =>
            [...current.filter((m) => m.id !== reply.id), reply].slice(-12),
          ),
      }),
    onMutate: (content) => {
  ...
  const unmute = useMutation({
    mutationFn: async (userId: string) => {
      if (!user || !roomId) throw new Error("غير مصرح");
      await liftRestriction(roomId, userId, "mute");
    },
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["room_moderation", roomId] });
  ...
  const block = useMutation({
    ...
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["user_blocks", user?.id] });
      toast.success("تم حظر العضو لديك");
    },
    onError: (error) => toast.error((error as Error).message),
  });
  function submit() {
    ...
    <div className="flex min-h-0 h-[calc(100dvh-115px)] gap-2 overflow-hidden pb-[calc(78px+env(safe-area-inset-bottom))] sm:h-[calc(100dvh-150px)] sm:pb-0 lg:h-[calc(100dvh-175px)] lg:gap-3">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2 scrollbar-slim">
          {(rooms.data ?? []).map((item) => (
            <Link
              ...
              className={`mb-1 flex items-center gap-2 rounded-xl px-2.5 py-2.5 transition ${item.slug === slug ? "bg-primary/15 text-primary shadow-sm" : "hover:bg-secondary/70"}`}
            >
      ...
      <section className="glass-strong flex min-w-0 flex-1 flex-col overflow-hidden rounded-2xl lg:rounded-3xl">
        <header className="flex shrink-0 items-center gap-2 border-b bg-background/35 px-2.5 py-2.5 sm:gap-3 sm:px-4 sm:py-3">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="truncate font-display text-sm font-black sm:text-base">
              {room.data.name}
            </h1>
            <p className="truncate text-[10px] text-muted-foreground sm:text-xs">
              {room.data.description ?? "غرفة ديوان"} · اضغط الاسم لعرض الأعضاء
            </p>
          </div>
          <Badge
            variant="secondary"
            className="hidden shrink-0 gap-1 px-2 text-[10px] sm:inline-flex sm:text-xs"
          >
            <span className="size-1.5 rounded-full bg-emerald-400" />
            {presence.online.length} {t.chat.online}
          </Badge>
          <div className="flex shrink-0 items-center gap-1.5">
            {canManageRoles ? (
              <Button
                ...
                size="sm"
                variant="outline"
                className="h-9 shrink-0 gap-1.5 rounded-xl border-primary/30 bg-primary/5 px-2 text-[10px] font-black text-primary sm:px-3 sm:text-xs"
                onClick={() => setRolesOpen(true)}
                aria-label="تعديل الرتب والصلاحيات"
              >
                <ShieldCheck className="size-4" />
                <span>الرتب</span>
                <span className="hidden sm:inline"> والصلاحيات</span>
              </Button>
            ) : null}
            <Button
              type="button"
              size="sm"
              variant="outline"
              className="h-9 shrink-0 gap-1.5 rounded-xl border-border/70 bg-background/50 px-2.5 text-[10px] font-bold sm:px-3 sm:text-xs lg:hidden"
              onClick={() => window.dispatchEvent(new Event("diwan:open-members"))}
              aria-label="فتح أعضاء الغرفة"
            >
        ...
        <RoomRolesDialog
          ...
          permissions={{ ...permissions, canManageRoles }}
          open={rolesOpen}
        ...
        <div
          ref={scrollRef}
          onScroll={onScroll}
          className="relative min-h-0 flex-1 overflow-y-auto px-2.5 py-3 scrollbar-slim"
        >
          <div className="mx-auto w-full max-w-4xl space-y-2.5">
            ...
            {messages.isLoading ? (
              ...
              items.map((message, index) => {
                ...
                const isAI = message.user_id.startsWith("demo-");
                const bubble = admin
                  ...
                  <div key={message.id} className="space-y-2">
                    {showDay ? (
                      <div className="flex items-center gap-2 py-1">
                        ...
                        <span className="rounded-full bg-secondary px-2.5 py-1 text-[9px] font-semibold text-muted-foreground">
                          {dayLabel(message.created_at, locale, t.chat)}
                        </span>
                        <span className="h-px flex-1 bg-border" />
                        <span className="h-px flex-1 bg-border" />
                      </div>
                    ...
                    <div className={`group flex items-end gap-2 ${mine ? "flex-row-reverse" : ""}`}>
                      ...
                      <div
                        className={`min-w-0 max-w-[88%] sm:max-w-[72%] ${mine ? "items-end" : "items-start"}`}
                      >
                        <div
                          className={`rounded-2xl px-3.5 py-2 ${mine ? "rounded-te-md" : "rounded-ts-md"} ${bubble}`}
                        >
                          <div className="mb-0.5 flex flex-wrap items-center gap-1.5">
                            {admin ? (
                              <span className="rounded-full border border-rose-400/40 bg-rose-500/10 px-1.5 py-0.5 text-[8px] font-black text-rose-300">
                                👑 ADMIN
                              </span>
                            ) : mod ? (
                              <span className="rounded-full border border-sky-400/40 bg-sky-500/10 px-1.5 py-0.5 text-[8px] font-black text-sky-300">
                                MOD
                              </span>
                            ) : null}
                            {isAI ? (
                              <p
                                className={
                                  admin
                                    ...
                                    : mod
                                      ? "text-[10px] font-bold text-sky-300"
                              ...
                              <Link
                                ...
                                className={
                                  admin
                                    ? "text-[10px] font-black text-rose-300 hover:underline"
                                    : mod
                                      ? "text-[10px] font-bold text-sky-300 hover:underline"
                                      : "text-[10px] font-semibold opacity-70 hover:underline"
                                }
                              >
                                {name}
                              </Link>
                          ...
                          {parent ? (
                            <p className="mb-1.5 truncate rounded-lg border-s-2 border-current/25 ps-2 text-[10px] opacity-65">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              variant="ghost"
                              size="icon"
                              className="size-7 self-center opacity-0 transition group-hover:opacity-100 focus:opacity-100"
                            >
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {!targetIsSelf && !mine && !isAI ? (
                              <>
                                <DropdownMenuItem
                                  onClick={() => toast.info("نظام الإبلاغ متاح من لوحة الإشراف")}
                                >
                                  <Flag className="size-4" />
                                  إبلاغ عن العضو
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() => block.mutate(message.user_id)}
                                ...
                                {canModerate ? (
                                  <>
                                    ...
                                    <DropdownMenuItem
                                      ...
                                      disabled={moderate.isPending || unmute.isPending}
                                    >
                                      <VolumeX className="size-4" />
                                      {targetIsMuted ? "إلغاء الكتم" : "كتم العضو"}
                                    </DropdownMenuItem>
                                    <DropdownMenuItem
                                      className="text-destructive"
                                      onClick={() =>
                                        moderate.mutate({ userId: message.user_id, kind: "ban" })
                                      }
        ...
        {unseen > 0 && !atBottom ? (
          <Button
            size="sm"
            className="absolute bottom-28 start-1/2 -translate-x-1/2 rounded-full shadow-lg"
            onClick={() => {
              setAtBottom(true);
        ...
        {replyTo ? (
          <div className="flex shrink-0 items-center gap-2 border-t bg-secondary/35 px-3 py-1.5 text-[10px] sm:px-4">
            <Reply className="size-3.5 shrink-0 text-primary" />
            <span className="min-w-0 flex-1 truncate">
              {replyTo.author?.display_name || replyTo.author?.username}: {replyTo.content}
            </span>
            <Button
              variant="ghost"
              size="icon"
        ...
        {isBanned || isMuted ? (
          <div className="shrink-0 border-t bg-destructive/10 p-3 text-sm text-destructive">
            <ShieldOff className="me-2 inline size-4" />
            {isBanned ? t.chat.banned : t.chat.muted}
          </div>
        ) : (
          <>
            <div className="shrink-0 border-t bg-background/30 px-2 pt-1.5">
