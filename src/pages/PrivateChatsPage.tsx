...
export function PrivateChatsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {chats.isLoading ? <MessagesSkeleton /> : (chats.data ?? []).length === 0 ? (
        <EmptyState icon={MessagesSquare} title="لا توجد محادثات خاصة" description="ابدأ محادثة خاصة من بروفايل عضو." />
      ) : (
        <ul className="space-y-2">
          {(chats.data ?? []).map((chat) => {
            ...
            return (
              <li key={chat.id}>
                <Link to="/chat/$slug" params={{ slug: chat.slug }} className="glass flex items-center gap-3 rounded-2xl p-4">
