...
export function PublicProfilePage() {
  ...
  const bot = aiMembers.find((member) => member.id === userId);
  ...
  if (bot) return <div className="mx-auto w-full max-w-2xl space-y-5"><Link to="/chat"><Button variant="ghost" size="sm"><ArrowRight className="me-2 size-4" />العودة</Button></Link><section className="glass-strong relative overflow-hidden rounded-3xl p-6"><div className="pointer-events-none absolute -end-20 -top-20 size-56 rounded-full bg-fuchsia-400/10 blur-3xl" /><div className="relative flex flex-col items-center gap-4 text-center"><div className="grid size-28 place-items-center rounded-full border-4 border-fuchsia-400/40 bg-secondary text-6xl shadow-xl">{bot.avatar}</div><div><div className="flex flex-wrap items-center justify-center gap-2"><h1 className="font-display text-2xl font-black">{bot.name}</h1><Badge className="border-fuchsia-400/40 bg-fuchsia-400/15 text-fuchsia-300"><Bot className="me-1 size-3" />AI</Badge></div><p className="mt-1 text-sm text-muted-foreground">@{bot.id}</p></div><div className="w-full rounded-2xl border bg-secondary/30 p-4 text-start"><p className="mb-1 text-xs font-semibold text-muted-foreground">النبذة</p><p className="text-sm leading-6">شخصية ذكاء اصطناعي في ديوان · {bot.personality}</p><p className="mt-2 text-xs text-muted-foreground">المواضيع: {bot.topics.join(" · ")}</p></div></div></section></div>;
  if (profile.isLoading) return <div className="p-6 text-center text-muted-foreground">جارٍ تحميل الملف الشخصي...</div>;
  if (!profile.data) return <div className="space-y-4 p-6 text-center"><UserRound className="mx-auto size-12 text-muted-foreground" /><h1 className="font-display text-xl font-bold">العضو غير موجود</h1><Link to="/chat"><Button variant="outline">العودة للدردشة</Button></Link></div>;
  const role = identity.data?.role;
  const avatarRole = role === "admin" ? "admin" : role === "moderator" ? "moderator" : null;
  return <div className="mx-auto w-full max-w-2xl space-y-5"><Link to="/chat"><Button variant="ghost" size="sm"><ArrowRight className="me-2 size-4" />العودة</Button></Link><section className="glass-strong relative overflow-hidden rounded-3xl p-6"><div className="pointer-events-none absolute -end-20 -top-20 size-56 rounded-full bg-primary/10 blur-3xl" /><div className="relative flex flex-col items-center gap-4 text-center"><div className="rounded-full bg-gradient-to-br from-primary via-fuchsia-400 to-amber-300 p-1.5 shadow-xl"><UserAvatar name={profile.data.display_name || profile.data.username} src={profile.data.avatar_url} size="lg" role={avatarRole} /></div><div><div className="flex flex-wrap items-center justify-center gap-2"><h1 className="font-display text-2xl font-black">{profile.data.display_name || profile.data.username}</h1>{role === "admin" ? <Badge className="border-amber-400/40 bg-amber-400/15 text-amber-300"><ShieldCheck className="me-1 size-3" />ADMIN</Badge> : role === "moderator" ? <Badge className="border-sky-400/40 bg-sky-400/15 text-sky-300">MOD</Badge> : null}</div><p className="mt-1 text-sm text-muted-foreground">@{profile.data.username}</p></div><div className="w-full rounded-2xl border bg-secondary/30 p-4 text-start"><p className="mb-1 text-xs font-semibold text-muted-foreground">النبذة</p><p className="whitespace-pre-wrap text-sm leading-6">{profile.data.bio || "هذا العضو لم يضف نبذة بعد."}</p></div></div></section></div>;
}
