...
export function PublicProfilePage() {
      ...
      <div className="mx-auto w-full max-w-2xl space-y-5">
        ...
        <section className="glass-strong relative overflow-hidden rounded-3xl p-6">
          ...
          <div className="relative flex flex-col items-center gap-4 text-center">
            <div className="grid size-28 place-items-center rounded-full border-4 border-fuchsia-400/40 bg-secondary text-6xl shadow-xl">
              {bot.avatar}
            </div>
            <div>
              <h1 className="font-display text-2xl font-black">{bot.name}</h1>
              <p className="mt-1 text-sm text-muted-foreground">@{bot.id}</p>
            </div>
            ...
            <div className="w-full rounded-2xl border bg-secondary/30 p-4 text-start">
              ...
              <p className="text-sm leading-6">
                {bot.personality} · يحب {bot.topics.slice(0, 3).join("، ")}.
              </p>
            </div>
          </div>
        </section>
  ...
  if (!profile.data || profile.data.id !== userId)
    return (
  ...
  const busy =
    addFriend.isPending || acceptFriend.isPending || removeFriend.isPending || message.isPending;
          ...
          {!isSelf ? (
            <div className="flex w-full flex-wrap justify-center gap-2">
              {relation?.status === "accepted" ? (
                ...
                <Button className="gap-2" onClick={() => acceptFriend.mutate()} disabled={busy}>
