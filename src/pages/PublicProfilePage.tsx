...
export function PublicProfilePage() {
  ...
  return <div className="mx-auto w-full max-w-2xl space-y-5">
    ...
    <section className="glass-strong relative overflow-hidden rounded-3xl p-6"><div className="pointer-events-none absolute -end-20 -top-20 size-56 rounded-full bg-primary/10 blur-3xl" /><div className="relative flex flex-col items-center gap-4 text-center">
      <div className="rounded-full bg-gradient-to-br from-primary via-fuchsia-400 to-amber-300 p-1.5 shadow-xl"><UserAvatar name={profile.data.display_name || profile.data.username} src={profile.data.avatar_url} size="lg" role={avatarRole} /></div>
      <div><div className="flex flex-wrap items-center justify-center gap-2"><h1 className="font-display text-2xl font-black">{profile.data.display_name || profile.data.username}</h1>{role === "admin" ? <Badge className="border-amber-400/40 bg-amber-400/15 text-amber-300"><ShieldCheck className="me-1 size-3" />ADMIN</Badge> : role === "moderator" ? <Badge className="border-sky-400/40 bg-sky-400/15 text-sky-300">MOD</Badge> : null}</div><p className="mt-1 text-sm text-muted-foreground">@{profile.data.username}</p></div>
