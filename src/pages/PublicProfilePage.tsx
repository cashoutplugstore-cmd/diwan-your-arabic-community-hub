...
export function PublicProfilePage({ userId }: { userId: string }) {
  ...
  const bot = aiBot ?? (demoMember ? {
    id: demoMember.id,
    name: demoMember.display_name || demoMember.username,
    avatar: "🤖",
    personality: "عضو من مجتمع ديوان",
    topics: ["السوالف", "المجتمع", "الدردشة"],
  } : null);
  ...
  if (profile.isPending) return <div className="p-6 text-center text-muted-foreground">جارٍ تحميل الملف الشخصي...</div>;
  if (profile.isError) return <div className="space-y-4 p-6 text-center"><UserRound className="mx-auto size-12 text-muted-foreground" /><h1 className="font-display text-xl font-bold">تعذر تحميل الملف الشخصي</h1><p className="text-sm text-muted-foreground">حاول فتح الملف مرة أخرى.</p><Link to="/chat"><Button variant="outline">العودة للدردشة</Button></Link></div>;
  ...
  const displayName = profile.data.display_name || profile.data.username || "عضو ديوان";
  return <div className="mx-auto w-full max-w-2xl space-y-5">
    <Link to="/chat"><Button variant="ghost" size="sm"><ArrowRight className="me-2 size-4" />العودة</Button></Link>
    <section className="glass-strong relative overflow-hidden rounded-3xl p-6">
      <div className="relative flex flex-col items-center gap-4 text-center">
        <div className="rounded-full bg-gradient-to-br from-primary via-fuchsia-400 to-amber-300 p-1.5 shadow-xl"><UserAvatar name={displayName} src={profile.data.avatar_url} size="lg" autoCurrentRole={false} /></div>
        <div><h1 className="font-display text-2xl font-black">{displayName}</h1><p className="mt-1 text-sm text-muted-foreground">@{profile.data.username}</p></div>
